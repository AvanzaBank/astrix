...
public class AstrixServiceProxy<T> {
	
	private ServiceFactory<T> serviceFactory;
	private InvocationHandler state;
	
	public static <T> T create(Class<T> api) {
		return (T) Proxy.newProxyInstance(AstrixServiceProxy.class.getClassLoader(), new Class[]{api}, new StatefulInvocationHandler(new Disconnected<>()));
	}
