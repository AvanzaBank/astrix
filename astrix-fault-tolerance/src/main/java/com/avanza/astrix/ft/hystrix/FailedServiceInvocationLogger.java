...
final class FailedServiceInvocationLogger extends HystrixEventNotifier {
	...
	public FailedServiceInvocationLogger(BeanMapping beanMapping) {
	...
	private void logEvent(HystrixEventType eventType, HystrixCommandKey key) {
		switch (eventType) {
			...
			case SHORT_CIRCUITED:
				log.info(String.format("Aborted command execution: cause=%s astrixBean=%s hystrixCommandKey=%s", eventType, beanMapping.getBeanKey(key).orElse(null), key.name()));
				break;
			default:
				// Do nothing
		}
